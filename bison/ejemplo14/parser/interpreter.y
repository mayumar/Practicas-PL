...
%union {
  ...
  std::list<double>  *parameters;  // New in example 14; NOTE: #include<list> must be in interpreter.l, init.cpp, interpreter.cpp
}
	...
	| BUILTIN LPAREN listOfExp RPAREN
		{
			...
			// Check the number of parameters equality
			if (f->getNParameters() ==  (int) $3->size())
			{
				switch(f->getNParameters())
				{
					case 0:
							...
							lp::BuiltinParameter0 *f0 = (lp::BuiltinParameter0 *) table.getSymbol($1);
					...
					case 1:
							...
							lp::BuiltinParameter1 *f1 = (lp::BuiltinParameter1 *) table.getSymbol($1);
					...
					case 2:
							...
							lp::BuiltinParameter2 *f2 = (lp::BuiltinParameter2 *) table.getSymbol($1);
							...
							n2 = $3->front();
							// Apply the function to the parameters and copy the result
					    	$$ = f2->getFunction()(n1,n2);
							}
							break;

					default:
				  			 execerror("Too many parameters ", f->getName());							
				} 
			}
			else
	  			 execerror("Incompatible number of parameters for function", f->getName());	
		}
;
...
listOfExp: 
	...
	|  exp restOfListOfExp
			{
				$$ = $2;
				// Insert the value of the first expression in the list of the rest of expressions
